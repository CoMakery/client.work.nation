...
const contractAddress = '0x3f79a56038c339bc87853ca17026c3eddf9cfa9b'
...
export default class UportUser {
  ...
  static createEthereumClaim(...ipfsKeys) {
    ...
    d(...ipfsKeys)
    return new Promise((resolve, reject) => {
      if (ipfsKeys.length > 2) return reject('Max 2 keys supported')
      // this hack is necessary because solidity function overloading seems
      // not correctly supported by the browser libraries operating here:
      const putFunction = claims[`put${ipfsKeys.length}`]  // Claim#put1 or Claim#put2
      putFunction(...ipfsKeys, (error, txhash) => {
        if (error) {
          return reject(error)
        } else {
